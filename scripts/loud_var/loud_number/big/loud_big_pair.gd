class_name LoudBigPair
extends Resource
## New version of BigFloatPair
...
func _init(base_value = 1.0, base_total = base_value) -> void:
	current = LoudBig.new(base_value)
	total = LoudBig.new(base_total)
...
func check_if_full() -> void:
	if is_full():
		if cap_current and current.is_greater_than(get_total()):
			fill()
		filled.emit()
...
func do_not_cap_current() -> LoudBigPair:
	cap_current = false
	return self
...
func tie_node_visibility(control: Control, visible_only_when_full: bool = true) -> void:
...
func get_midpoint() -> Big:
	if is_full() and cap_current:
		return get_total()
	var sum: Big = current.plus(get_total())
