...
static func get_easter_date(easter_year: int) -> Dictionary[StringName, int]:
	var a: int = easter_year % 19
	var b: int = floori(easter_year / 100.0)
	var c: int = easter_year % 100
	...
	var easter_month: int = floori((h + l - 7 * m + 114) / 31.0)
	var easter_day: int = ((h + l - 7 * m + 114) % 31) + 1
...
static func get_easter_datetime(_year: int) -> Dictionary:
	var easter: Dictionary[StringName, int] = get_easter_date(_year)
	return {
		"year": easter.easter_year,
		"month": easter.easter_month,
		"day": easter.easter_day,
		"hour": 0,
		"minute": 0,
		"second": 0,
	}
...
func _connect_signals() -> void:
	...
	SaveManager.loading.became_false.connect(recalc_day_and_month)
...
func get_date_text() -> String:
	return "%s, %s %s, %s" % [
		Day.keys()[day].capitalize(),
		Month.keys()[month].capitalize(),
		day_count,
		year_count,
	]
